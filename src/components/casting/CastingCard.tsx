...
export const CastingCard: React.FC<CastingCardProps> = ({ 
  ...
  return (
    <div className="rounded-xl bg-[#0c0d12] border border-amber-500/20 hover:border-amber-400/50 hover:shadow-[0_0_20px_rgba(245,158,11,0.12)] transition-all p-5 shadow-lg flex flex-col justify-between group">
      <div>
        {/* Top bar: Company & Badges */}
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-500/10 text-amber-300 border border-amber-500/25">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Heart className={`w-4 h-4 transition-transform active:scale-75 ${isSaved ? 'fill-amber-400 text-amber-400' : ''}`} />
            </button>
          </div>
        </div>

        {/* Title */}
        <Link to={`/casting/${castingCall.id}`} className="group-hover:text-amber-300 transition-colors">
        ...
        <div className="space-y-2 mb-4">
          <div className="flex items-center justify-between text-xs text-zinc-400 font-medium">
            ...
            <button
              ...
              className="text-amber-400 hover:text-amber-300 text-xs flex items-center gap-0.5 font-medium"
            >
          ...
          <div className="space-y-2">
            {(expanded ? roles : roles.slice(0, 2)).map(role => (
              <div 
                key={role.id} 
                className="p-2.5 rounded-lg bg-black/60 border border-amber-500/15 flex items-center justify-between gap-3 text-xs"
              >
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] capitalize text-amber-300/90 font-medium">
                ...
                {onApplyClick && (
                  <button
                    ...
                    className="px-3 py-1 rounded-lg bg-gradient-to-r from-amber-400 via-amber-500 to-yellow-500 hover:from-amber-300 hover:to-amber-400 text-zinc-950 font-bold text-xs whitespace-nowrap transition-all shadow-sm active:scale-95"
                  >
                    Apply
                  </button>
                )}
      ...
      {/* Footer Info */}
      <div className="pt-3 border-t border-amber-500/15 flex items-center justify-between text-xs text-zinc-400">
