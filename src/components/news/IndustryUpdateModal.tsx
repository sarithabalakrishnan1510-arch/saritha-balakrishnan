...
export const IndustryUpdateModal: React.FC<IndustryUpdateModalProps> = ({
  ...
  const getCategoryBadge = (category: string) => {
    switch (category) {
      case 'press_release':
        return { label: 'Official Press Release', bg: 'bg-rose-500/10 text-rose-300 border-rose-500/30' };
      case 'guild_directive':
        return { label: 'Guild Directive (FEFKA/AMMA)', bg: 'bg-amber-500/10 text-amber-300 border-amber-500/30' };
      ...
      case 'trade_boxoffice':
        return { label: 'Trade & Box Office Wire', bg: 'bg-blue-500/10 text-blue-300 border-blue-500/30' };
  ...
  return (
    ...
    >
      <div 
        className="relative w-full max-w-3xl rounded-2xl bg-[#11131c] border border-white/15 shadow-2xl overflow-hidden my-8"
        onClick={e => e.stopPropagation()}
      >
        {/* Banner image if present */}
        {update.image_url && (
          <div className="h-48 w-full relative overflow-hidden bg-zinc-900 border-b border-white/10">
            <img 
            ...
            <div className="absolute top-4 right-4 flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={onClose}
                className="p-2 rounded-xl bg-black/60 hover:bg-black/90 text-zinc-300 hover:text-white border border-white/20 backdrop-blur-md transition-colors"
        ...
        <div className="p-6 sm:p-8 space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-4">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {update.urgent && (
                <span className="px-2.5 py-1 rounded-full text-[11px] font-bold bg-rose-600/20 text-rose-300 border border-rose-500/40 flex items-center gap-1">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-2.5 py-1 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-[11px] font-mono text-zinc-300 hover:text-white flex items-center gap-1.5 transition-colors"
          ...
          {update.signatory && (
            <div className="p-4 rounded-xl bg-black/40 border border-white/10 flex items-center justify-between gap-4 text-xs">
              <div>
              ...
              <div className="text-right">
                <span className="inline-flex items-center gap-1 px-2 py-1 rounded bg-emerald-500/10 text-emerald-300 text-[11px] font-semibold border border-emerald-500/30">
          ...
          <div className="flex flex-wrap items-center gap-1.5 pt-2 border-t border-white/5">
            ...
            {update.tags.map(tag => (
              <span 
                key={tag}
                className="text-[11px] px-2 py-0.5 rounded bg-white/5 text-zinc-400 border border-white/5 font-medium"
              >
                #{tag}
              </span>
            ))}
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t border-white/10">
            <div className="flex items-center gap-2 w-full sm:w-auto">
              ...
              {update.official_url && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-zinc-300 hover:text-white text-xs flex items-center gap-1.5 transition-colors"
                >
            ...
            <button
              type="button"
              onClick={onClose}
              className="w-full sm:w-auto px-5 py-2 rounded-xl bg-white/10 hover:bg-white/15 text-white font-semibold text-xs transition-colors text-center"
            >
              Close Dispatch
            </button>
          </div>
