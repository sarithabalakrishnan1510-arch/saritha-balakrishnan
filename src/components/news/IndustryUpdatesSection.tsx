  ...
  return (
    <section id="industry-updates" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-white/10 pb-6">
        <div className="space-y-2 max-w-2xl">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold tracking-wider uppercase bg-rose-500/10 text-rose-300 border border-rose-500/30 flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <button
            ...
            disabled={refreshing}
            className="px-3.5 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-zinc-300 hover:text-white text-xs font-semibold flex items-center gap-2 transition-colors disabled:opacity-50"
      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl font-medium whitespace-nowrap transition-all flex items-center gap-1.5 ${
                activeCategory === cat.id
                  ? 'bg-rose-600 text-white shadow-lg shadow-rose-950/40 border border-rose-500/40'
        ...
        <div className="relative w-full lg:w-72 shrink-0">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-1.5 rounded-xl bg-white/5 border border-white/10 text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-rose-500/50"
          />
        </div>
      </div>
      ...
      {loading && updates.length === 0 ? (
        ...
        /* Empty State */
        <div className="p-12 text-center rounded-2xl bg-[#12141d] border border-white/5 space-y-3">
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/15 text-white text-xs font-semibold transition-colors mt-2"
          >
            Clear Filters
          </button>
        </div>
        ...
        <div className="space-y-6">
          {/* Featured / Statutory Bulletin Card */}
          {featuredUpdate && !searchQuery && activeCategory === 'all' && (
            <div className="rounded-2xl bg-gradient-to-r from-rose-950/30 via-[#131520] to-[#12141c] border border-rose-500/30 p-6 sm:p-8 relative overflow-hidden shadow-2xl">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-center relative z-10">
                <div className="lg:col-span-2 space-y-3">
                  ...
                  <h3 className="text-xl sm:text-2xl font-bold text-white font-cinematic hover:text-rose-300 transition-colors cursor-pointer leading-snug"
                  ...
                  <div className="flex flex-wrap items-center gap-y-2 gap-x-4 pt-2 text-xs">
                    <button
                      type="button"
                      onClick={() => handleOpenUpdate(featuredUpdate)}
                      className="px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-semibold text-xs flex items-center gap-1.5 transition-colors shadow-lg"
                    >
                ...
                {featuredUpdate.image_url && (
                  <div 
                    className="relative h-44 rounded-xl overflow-hidden border border-white/10 group cursor-pointer"
                    onClick={() => handleOpenUpdate(featuredUpdate)}
                  >
                    <img 
                    ...
                    <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-[11px] text-white">
                      <span className="font-semibold bg-black/60 px-2 py-0.5 rounded backdrop-blur-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {(searchQuery || activeCategory !== 'all' ? updates : regularUpdates).map(item => {
              ...
              return (
                <article
                  key={item.id}
                  className="rounded-2xl bg-[#12141d] border border-white/10 hover:border-rose-500/30 transition-all p-5 flex flex-col justify-between space-y-4 shadow-xl group"
                >
                  <div className="space-y-3">
                    ...
                    <h3 
                      ...
                      className="text-sm font-bold text-white group-hover:text-rose-300 transition-colors cursor-pointer leading-snug line-clamp-2"
                    >
                      {item.title}
                    </h3>
                    ...
                    {item.key_takeaways && item.key_takeaways[0] && (
                      <div className="p-2 rounded-lg bg-black/40 border border-white/5 flex items-start gap-1.5 text-[11px] text-zinc-300">
