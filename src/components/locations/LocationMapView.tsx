...
export const LocationMapView: React.FC<LocationMapViewProps> = ({
    ...
    locations.forEach((loc) => {
      ...
      const markerHtml = `
        <div class="group relative flex items-center justify-center cursor-pointer transition-transform duration-200 ${
          ...
          ${
            isSelected
              ? `<div class="absolute -inset-2 bg-rose-500/40 rounded-full animate-ping pointer-events-none"></div>`
              : ''
          }
          <div class="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold shadow-xl border backdrop-blur-md ${
            isSelected
              ? 'bg-rose-600 text-white border-white/60 ring-2 ring-rose-400/80'
              : 'bg-[#12141c]/95 text-zinc-100 border-white/20 hover:border-amber-400/80 hover:bg-[#1c1f2b]'
          }">
            <span class="w-2 h-2 rounded-full ${
              loc.night_shoot_allowed ? 'bg-indigo-400' : 'bg-emerald-400'
            }"></span>
            <span class="truncate max-w-[110px] text-[11px]">${loc.city}</span>
            <span class="text-[10px] font-mono px-1 rounded bg-black/40 text-amber-300">${loc.pricing_text.split('/')[0].trim()}</span>
          </div>
          ${
            isFav
              ? `<span class="absolute -top-1 -right-1 w-3.5 h-3.5 bg-rose-600 rounded-full flex items-center justify-center text-[8px] text-white border border-white">♥</span>`
              : ''
          }
        </div>
      `;
  ...
  return (
    <div className={`relative w-full rounded-2xl overflow-hidden border border-white/10 bg-[#0d0e12] shadow-2xl flex flex-col ${className}`}>
      {/* Top Map Control Bar */}
      <div className="relative z-10 flex flex-wrap items-center justify-between gap-2 p-3 sm:px-4 bg-[#141620]/95 backdrop-blur-md border-b border-white/10 text-xs">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="hidden md:flex items-center gap-1 overflow-x-auto max-w-md py-0.5">
            {POPULAR_HUBS.slice(0, 4).map((hub) => (
              <button
                ...
                className="px-2.5 py-1 rounded-lg bg-white/5 hover:bg-white/10 text-zinc-300 hover:text-white border border-white/10 text-[11px] transition-colors whitespace-nowrap"
              >
                {hub.label}
              </button>
            ))}
          ...
          <div className="relative">
            ...
            {isLayerMenuOpen && (
              <div className="absolute right-0 top-full mt-1.5 w-48 rounded-xl bg-[#181a24] border border-white/15 p-1.5 shadow-2xl z-50 space-y-1">
      ...
      <div className="relative flex-1 w-full min-h-[520px] md:min-h-[580px] lg:min-h-[640px]">
        ...
        <div className="md:hidden absolute top-3 left-3 right-3 z-10 flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none">
          {POPULAR_HUBS.map((hub) => (
            <button
              ...
              type="button"
              onClick={() => handleHubSelect(hub)}
              className="px-2.5 py-1 rounded-full bg-[#12141c]/90 backdrop-blur-md text-zinc-200 border border-white/15 text-[11px] font-medium whitespace-nowrap shadow-lg shrink-0"
            >
              {hub.label}
            </button>
          ))}
        ...
        {activeLocation ? (
          <div className="absolute bottom-4 left-3 right-3 sm:left-auto sm:right-4 sm:w-96 max-w-full z-30 animate-in fade-in slide-in-from-bottom-3 duration-200">
            <div className="rounded-2xl bg-[#12141f]/95 backdrop-blur-xl border border-white/20 p-4 shadow-2xl text-white space-y-3">
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="p-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-rose-400 transition-colors"
                    title="Toggle Favorite"
                  >
                    <Heart
                      className={`w-4 h-4 ${
                        isFavoriteLocation(activeLocation.id) ? 'fill-rose-500 text-rose-500' : ''
                      }`}
                    />
                  </button>
                  <button
              ...
              {/* Action Buttons */}
              <div className="pt-1 flex items-center gap-2 border-t border-white/10">
                <button
                  type="button"
                ...
                <button
                  ...
                  className="flex-1 py-2 px-3 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-semibold text-xs transition-colors shadow-lg shadow-rose-900/30 flex items-center justify-center gap-1.5"
                >
          ...
          <div className="absolute bottom-4 left-4 z-10 pointer-events-none hidden sm:block">
            <div className="px-3 py-1.5 rounded-xl bg-[#12141c]/90 backdrop-blur-md border border-white/10 text-zinc-400 text-xs flex items-center gap-2 shadow-xl">
      ...
      <div className="p-3 bg-[#11131a] border-t border-white/10 overflow-x-auto flex items-center gap-2.5 scrollbar-thin">
        <span className="text-[11px] font-bold uppercase tracking-wider text-zinc-400 whitespace-nowrap pl-1">
          ...
          return (
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs flex items-center gap-2 border transition-all whitespace-nowrap shrink-0 ${
                isSelected
                  ? 'bg-rose-600 text-white border-rose-400 shadow-md scale-105 font-bold'
                  : 'bg-white/5 hover:bg-white/10 text-zinc-300 hover:text-white border-white/10 font-medium'
              }`}
            >
